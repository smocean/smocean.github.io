...
每个插件是一个npm模块，其名字按照这样的格式：`eslint-plugin-<plugin-name>`，比如`eslint-plugin-jquery`。你也可以限定包作用域下，按照这样的格式：`@<scope>/eslint-plugin-<plugin-name>`，比如`@jquery/eslint-plugin-jquery`。
...
如果的插件有规则，它必须输出一个带有`rules`属性的对象。这个`rules`属性应该是一个对象，该对象包含规则id和对应规则的键值对。这个规则ID不需要遵循任何命名规范（所以，比如，它可以是`dollar-sign`）。
...
你也可以创建插件告诉ESLint如何处理文件而不是Javascript。为了创建一个处理器，从你的模块中书输出的对象必须符合以下接口：
...
`preprocess`将文件内容和文件名称作为参数，返回一个要检查的字符串数组。这些字符串将被单独检查，但仍要注册到文件名。是否需要只返回一部分或是多个块，取决于这个插件。例如，在处理`.html`文件时，通过合并所有的脚本，你可能希望返回数组中的一项；但是对于`.md`文件，每个javascript块可能是独立的，你可以返回多个项。
...
`postprocess`方法需要一个二维数组作为参数，用于检查消息和文件名。数组的中的每一项对应着从`preprocess`方法返回的部分。`postprocess`方法必须调整所有错误的位置并将他们汇集到一个的扁平的数组中，然后返回该数组。
...
你可以在你的插件中通过输出一个包含`rulesConfig`属性的对象为规则提供默认的配置。同你使用你的.eslintrc 配置中`rules`属性的一样，`rulesConfig`遵循同样的模式，但是没有插件名作为前缀。
...
为了明确插件要需要ESLint才能正常运行，你必须在你的`package.json`声明将ESLint作为一个`peerDependency`。对插件的支持在ESLint`0.8.0`版本中被引入。要确保`peerDependency`指向ESLint `0.8.0`或之后的版本。
...
你可以使用[`ESLintTester`](https://github.com/eslint/eslint-tester)测试你的插件的规则[同测试ESLint规则一样](working-with-rules)。
...
将这些关键字添加到你的`package.json`，会让其他人更容易找到它。
